...
(function () {
    ...
    WinJS.UI.Pages.define("./resources/pages/orders/orders.jsp", {
        ...
        _extrasHandler: function (args) {
            popup("orderExtrasPopup", "Manage Extras", "./resources/pages/extras/extras.jsp", this.currentItem.__quote);
        },
        _eventsHandler: function (args) {
            popup("orderExtrasPopup", "Manage Events", "./resources/pages/orderevents/orderevents.jsp", this.currentItem);
        },
        _buttonHandler: function (args) {
            ...
            switch (args.label) {
                case 'save': {
                    Data.orderSave(this.currentItem, this.originalItem).then(function (saved) {
                        if (saved) {
                            ...
                            if(index == -1)
                            { 
                            	index = Data.getIndexOfObject(saved, Data.orders, 'orders');
                            	if(index > -1)
                            	{
                            		Data.orders.setAt(index, saved);
                            	}
                            }
                ...
                case 'edit': {
                    nav.navigate("resources/pages/deliveries/deliveries.jsp", { order: that.currentItem });
                    break;
                }
                case 'delete': {
