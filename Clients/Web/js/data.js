(function () {
    ...
    function deliveriesGet() {
        ...
        return WinJS.xhr({ url: shipmentBaseAddress + "/api/shipments", headers: rest_headers }).then(function (response) {
    ...
    function ordersGet(dealer, status) {
        ...
        return WinJS.xhr({ url: orderBaseAddress + "/api/orders?dealer=" + encodeURIComponent(dealer) + "&status=" + encodeURIComponent(status), headers: rest_headers }).then(function (response) {
    ...
    function orderDelete(order) {
        return WinJS.xhr({
            type: 'delete',
            url: orderBaseAddress + "/api/orders/" + encodeURIComponent(order.orderId)
        }).then(function (response) {
            if (response.status == 204) {
    ...
    function orderGetById(orderId) {
        ...
        return WinJS.xhr({ url: orderBaseAddress + "/api/orders/" + encodeURIComponent(orderId) + "?unique=" + Date.now(), headers: rest_headers }).then(function (response) {
            Data.orders.splice(0, Data.orders.length);
    ...
    function orderAddEvent(orderId, event) {
        return WinJS.xhr({
            type: 'post',
            url: orderBaseAddress + "/api/orders/" + encodeURIComponent(orderId) + "/events",
            headers: rest_headers,
    ...
    function orderSaveEdits(order, original) {
        ...
        return orderSaveEditsToStatus(order, original).then(function () {
            ...
            return WinJS.xhr({
                type: 'put',
                url: orderBaseAddress + "/api/orders/" + encodeURIComponent(raworder.orderId),
                headers: rest_headers,
    ...
    function orderSaveEditsToStatus(order, original) {
        ...
        if (order.status != original.status) {
            statusSave = WinJS.xhr({
                type: 'put',
                url: orderBaseAddress + "/api/orders/" + encodeURIComponent(order.orderId) + "/status",
                headers: rest_headers,
    ...
    function orderSaveNew(order) {
        return WinJS.xhr({
            type: 'post',
            url: orderBaseAddress + "/api/orders",
            headers: rest_headers,
    ...
    function orderCreateFromQuote(quote) {
        return WinJS.xhr({
            type: 'post',
            url: orderBaseAddress + "api/orders?fromQuote=" + encodeURIComponent(quote.quoteId),
            headers: rest_headers,
            data: JSON.stringify({}),
        }).then(function (response) {
            if (response.status == 201) {
    ...
    function quotesGet(search) {
        ...
        return WinJS.xhr({ url: quoteBaseAddress + "/api/quotes?name=" + encodeURIComponent(search)  + "&unique=" + Date.now(), headers: rest_headers }).then(function (response) {
            if (response.status == 0) { 
    ...
    function quoteDelete(quote) {
        return WinJS.xhr({
            type: 'delete',
            url: quoteBaseAddress + "/api/quotes/" + encodeURIComponent(quote.quoteId)
        }).then(function (response) {
            if (response.status == 204) {
    ...
    function quoteGetById(quoteId) {
        ...
        return WinJS.xhr({ url: quoteBaseAddress + "/api/quotes/" + encodeURIComponent(quoteId) + "?unique=" + Date.now(), headers: rest_headers }).then(function (response) {
    ...
    function quoteSaveNew(quote) {
        ...
        return WinJS.xhr({
            type: 'post',
            url: quoteBaseAddress + "/api/quotes",
            headers: rest_headers,
            data: JSON.stringify(rawquote),
        }).then(function (response) {
            if (response.status == 201) {
    ...
    function catalogGet() {
        ...
        return WinJS.xhr({ url: catalogBaseAddress + "/api/catalog?unique=" + Date.now(), headers: rest_headers }).then(function (response) {
            if (response.status == 0) {
    ...
    function catalogDelete(product) {
        return WinJS.xhr({
            type: 'delete',
            url: catalogBaseAddress + "/api/catalog/" + encodeURIComponent(product.skuNumber)
        }).then(function (response) {
            if (response.status == 204) {
    ...
    function catalogSaveEdits(product) {
        ...
        return WinJS.xhr({
            type: 'put',
            url: catalogBaseAddress + "/catalog/" + encodeURIComponent(product.skuNumber),
            headers: rest_headers,
    ...
    function catalogSaveNew(product) {
        ...
        return WinJS.xhr({
            type: 'post',
            url: catalogBaseAddress + "api/catalog",
            headers: rest_headers,
            data: JSON.stringify(rawProduct),
        }).then(function (response) {
            if (response.status == 201) {
